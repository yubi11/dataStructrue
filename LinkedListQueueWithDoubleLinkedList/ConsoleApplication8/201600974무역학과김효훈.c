...
// 기본설명: ATM 3대가 큐를 이루고 있음, 각 ATM마다 front와 rear를 가지고 있고, 이 front와 rear는 Client 구조체를 가리킨다.
// Client구조체는 손님들의 정보를 나타내는 데이터로서, 큐의 요소이다. 또한, 이 Client구조체는 nextPerson, prevPerson이라는 이중연결리스트 노드를 가지고 있다.
// 한마디로, 이중연결리스트노드(Client)를 요소로 하는 double-edged 큐(ATM)를 의미한다.
...
void dequeueSpecific(LinkedQueueType* q, Client* temp)
{
	if (temp->prevPerson && temp->nextPerson == NULL)							// 지워지는 Client구조체가 맨뒤에 있는경우,
	{
		q->rear = temp->prevPerson;
		...
		if ((minutes % 10) == 0)				//매 10분마다, 소수번호를 가진사람들은 나가게됨,
		{
			...
			for (int j = 0; j < k; j++)
			{
				...
				if (isPrime(temp->number))		// 소수 확인
				{
					...
					dequeueSpecific((q + i), temp);												//dequeueMiddle 함수 사용하여 해당 노드 제거
